-- Query: https://dune.com/queries/8796538
-- Matview: dune.paltalabs.result_scf_users_roles_monthly   cron: 0 9 * * *
-- Última ejecución: 01M34WR6XAE7A1W5ZGCEW6FHYF
-- Costo: 1.089 cr; filas: 2587; engine medium
...
WITH metadata AS (
  ...
  FROM grid g LEFT JOIN activity a ON a.protocol = g.protocol
    AND a.activity_date >= g.period_start AND a.activity_date < date_add('month', 1, g.period_start) AND a.role = g.role
  GROUP BY 1, 2, 3, 4
), previous AS (
