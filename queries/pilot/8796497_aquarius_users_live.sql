-- Query: https://dune.com/queries/8796497
-- Matview: dune.paltalabs.result_scf_aquarius_users_live   cron: 0 5 * * *
-- Última ejecución: 01M32CXJCS3EAK2JRKCYHPG2JG
-- Costo: 25.228 cr; filas: 8368; engine medium
...
ev AS (
  ...
  WHERE he.contract_id IN (SELECT contract_id FROM routers)
    AND he.closed_at_date >= DATE '2026-06-01' AND he.closed_at_date < CURRENT_DATE
...
FROM users
UNION ALL
SELECT 'aquarius', CAST(NULL AS DATE), CAST(NULL AS VARCHAR), CAST(NULL AS VARCHAR),
