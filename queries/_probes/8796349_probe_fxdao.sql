-- Query: https://dune.com/queries/8796349
-- Matview: None   cron: None
-- Última ejecución: 01M32C1MEY0M4VZPXKR4ATWG5X
-- Costo: 0.793 cr; filas: 4; engine medium
WITH source AS (WITH ops AS (
  ...
  WHERE o.closed_at_date >= CURRENT_DATE - INTERVAL '180' DAY AND o.closed_at_date < CURRENT_DATE
...
tx AS (
  ...
  WHERE closed_at_date >= CURRENT_DATE - INTERVAL '180' DAY AND closed_at_date < CURRENT_DATE
    AND successful = TRUE
)
SELECT
  'fxdao' AS protocol,
